...
@php
    ...
    foreach($bookingInfo['hotel_room_details'] as $bookingRoomDetails){
        $cancelPolicy =
json_decode($bookingRoomDetails['cancellation_policy'], true);
...
<head>
    ...
    <style type="text/css">
        ...
        .booking-details-wrapper {
            ...
            -webkit-box-shadow: 0 0.125rem 0.25rem rgba(0, 0, 0,
0.075) !important;
...
__('apiMail.booking_confirmation_txt') }}</h1>
                                <p>{{ __('apiMail.thank_you_for_booking') }}</p>
                                <p>{{
__('apiMail.congratulations_find_your_booking') }}</p>
                                <p>{{
__('apiMail.if_you_have_query_contact_us') }}</p>
                                <p>{{ __('apiMail.booking_id') }}{{
$bookingInfo['booking_req_id'] }}</p>
              <!--                   <p>{{ __('apiMail.booking_pnr') }}{{
$bookingInfo['booking_ref_id'] }}</p> -->
                                <p>{{ __('apiMail.booking_date') }}{{
Common::getTimeZoneDateFormat($bookingInfo['created_at'],'Y',$portalTimeZone,config('common.mail_date_time_format'))
}}</p>
                            </td>
                ...
                <hr>
                <h4 class="mt-2 mb-3">{{
__('apiMail.itinery_reservation_details') }}</h4>
...
$bookingDetail['hotel_name'] }} @if($bookingDetail['star_rating'] > 0)
({{ $bookingDetail['star_rating']. ' Star'}}) @else ({{ $bookingDetail['star_rating'] }}) @endif </p>
                                        <p class="text-light-gray">{{
$bookingDetail['hotel_address'] }}</p>
                                        <p
class="text-light-gray">Contact No: {{ $hotelContactNo }}</p>
                                        <p
class="text-light-gray">Email: {{
$bookingDetail['hotel_email_address'] }}</p>
                                    </td>
                                    <td>
                                        <p class="text-light-gray">{{
$bookingDetail['check_in'] }} - {{ $bookingDetail['check_out'] }}</p>
                                        <p class="text-light-gray">{{
$bookingDetail['hotel_name'] }}</p>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
                ...
                <div class="flight-item">
                 <table class="table booking-pax-table">
                    ...
                    <tr>
                        <td class="px-2"><img src="{{
url('images/print/arrow-2.png') }}" width="10" class="pr-1"
alt="">{{$hotel_passenger['last_name']}}/{{$hotel_passenger['first_name']}}
{{$hotel_passenger['middle_name']}}</td>
                        <td class="px-2 border-gray 
border-left">{{ $hotelDetails['room_name'] }}</td>
                        <td class="px-2 border-gray 
border-left">{{ $hotelDetails['no_of_rooms'] }}</td>
                        <td class="px-2 border-gray 
border-left">{{ $hotelDetails['no_of_adult'] +
$hotelDetails['no_of_child'] }}</td>
                    </tr>
                </table>
               </div>
               <table class="table">
                    <tbody>
                        <tr>
                            <td>
                                <h3 class="mt-3 mb-3">{{
__('apiMail.payment_details') }}</h3>
                                <p>{{ __('apiMail.base_fare') }} :
{{$convertedCurrency.' '.Common::getRoundedFare($basefare *
$convertedExchangeRate)}}</p>

                                <p>{{ __('apiMail.Taxes_ans_Fees') }}
: {{$convertedCurrency.' '.Common::getRoundedFare(($tax) *
$convertedExchangeRate)}}</p>

                                @if($onflyDiscount != '' && $onflyDiscount != 0)
                                <p>{{ __('apiMail.discount') }} :
{{$convertedCurrency.' '.Common::getRoundedFare(($onflyDiscount) *
$convertedExchangeRate)}}</p>
                                @endif
                                ...
                                <p>{{ __('apiMail.convenience_fee') }}
: {{$convertedCurrency.' '.Common::getRoundedFare($paymentCharge *
$convertedExchangeRate)}}</p>

                                <p>{{ __('apiMail.total') }} :
{{$convertedCurrency.' '.Common::getRoundedFare(((($totalFare +
$onflyHst + $paymentCharge) - $promoDiscount) * $convertedExchangeRate))}}</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="passenger_details-pdf">
                    <h4 class="mt-3 mb-3">{{
__('apiMail.cancellation_policy') }}</h4>
                    <div class="flight-item">
                        <table class="table booking-pax-table ">
                            <tr>
                ...
                @if($bookingDetail['vat_info'] != null &&
$bookingDetail['vat_info'] != '')
                    <div class="booking-status-info">
                        <h4>{{ __('apiMail.hotel_note_text',
['vatInfoName' => $vatInfoName, 'vatInfoNumber' => $vatInfoNumber,
'referenceId' => $bookingInfo['booking_req_id'] ]) }}</h4>
                    </div>
                @endif
