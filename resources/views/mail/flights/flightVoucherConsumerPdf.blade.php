...
$segmentKey = 1;
$aSsrInfo   = Flights::getSsrDetails($flight_itinerary);

$baggageDisplay = false;
if(isset($aSsrInfo) && !empty($aSsrInfo)){
    $baggageDisplay = true;
}
$tmpBookingStatus = $booking_status;
if($ticket_status == '202'){
    $tmpBookingStatus = $ticket_status;
}
@endphp
            ...
            <tbody>
                <tr>
                    <td rowspan="2" class="bg-gray px-2" style="position: relative;">
                        ...
                        <p>Status: <br> {{isset($statusDetails[$tmpBookingStatus]) ? $statusDetails[$tmpBookingStatus] : '-'}}</p> 
                    </td>
                ...
                <tr>
                    <td class="px-2 border border-gray border-bottom-0 border-right-0 border-top">
                        ...
                        <p class="m-0">Terminal: <br> {{$segVal['departure_terminal']}}</p>
                    ...
                    <td class="px-2 border border-gray border-bottom-0 border-right-0 border-left border-top">
                        ...
                        <p class="m-0">Terminal: <br> {{$segVal['arrival_terminal']}}</p>
                    </td>
                </tr>
        ...
        <table class="w-100 mb-2 passenger-details-mail" style="border:1px solid #ccc;page-break-inside:avoid;">
            <thead>
                <tr>
                    ...
                    <td class="px-2 border-gray bg-gray border-left">Meal</td>
                    @if($baggageDisplay)<td class="px-2 border-gray bg-gray border-left">Baggage</td>@endif
            ...
            @foreach($flight_passenger as $paxKey => $paxVal)
                @php                    
                    ...
                    $segRefKey  = 'Segment'.$segmentKey;
                    $dispMeal   = '-'; 
                    $dispBaggage= '-'; 
                    if(isset($aSsrInfo) && !empty($aSsrInfo) && $paxVal['pax_type'] != 'INF' && $paxVal['pax_type'] != 'INS'){
                        foreach($aSsrInfo as $ssrKey => $ssrVal){
                            if($ssrVal['ServiceName'] != '' && $ssrVal['ServiceType'] == "MEAL" && $ssrVal['Origin'] == $segVal['departure_airport'] && $ssrVal['Destination'] == $segVal['arrival_airport'] && $ssrVal['PaxRef'] == $paxVal['pax_type'].$paxCount && $ssrVal['SegmentRef'] == $segRefKey){
                                $dispMeal = $ssrVal['ServiceName'];
                            }

                            if($ssrVal['ServiceName'] != '' && $ssrVal['ServiceType'] == "BAG" && $ssrVal['Origin'] == $segVal['departure_airport'] && $ssrVal['Destination'] == $segVal['arrival_airport'] && $ssrVal['PaxRef'] == $paxVal['pax_type'].$paxCount && $ssrVal['SegmentRef'] == $segRefKey){
                ...
                <tr>
                    ...
                    <td class="px-2 border-gray border-left">{{ $dispMeal }}</td>
                    @if($baggageDisplay)<td class="px-2 border-gray border-left">{{ $dispBaggage }}</td>@endif
