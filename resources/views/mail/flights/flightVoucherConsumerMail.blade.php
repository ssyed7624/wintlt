@php
    ...
    $displayFareRule = Flights::displayFareRule($account_id);
    extract($titleInfo);
...
<div class="container">
            ...
            <table class="table-sm w-100 even-width" border="0">
                <tbody>
                    <tr>
                        ...
                        @endif
                        <td>
                            <p class="m-0"><b>Status:</b> {{isset($statusDetails[$tmpBookingStatus]) ? $statusDetails[$tmpBookingStatus] : '-'}}</p>
        ...
        @if(count($seatsReq) > 0 || count($mealsReq) > 0 || count($aSsrInfo) > 0)
        @php 
            $ssrDetailsColspan = 2;
            $baggageDisplay = false;
            if(isset($aSsrInfo) && !empty($aSsrInfo) && count($aSsrInfo) > 0){
                $ssrDetailsColspan  = 3;
                $baggageDisplay     = true;
            }
        @endphp
                ...
                <tr>
                    ...
                    @if(isset($segmentAry) && !empty($segmentAry))
                        @foreach($segmentAry as $segTempVal)
                            <th colspan="{{$ssrDetailsColspan}}">{{$segTempVal['departure_airport']}} -> {{$segTempVal['arrival_airport']}} </th>
                        @endforeach
                    @endif
                </tr>
                <tr>
                    @if(isset($segmentAry) && !empty($segmentAry))
                        @for ($i = 0; $i < count($segmentAry); $i++)
                            <th>Seat</th>
                            <th>Meal</th>
                            @if($baggageDisplay)<th>Baggage</th>@endif
                        @endfor
                    @endif
                </tr>
                ...
                @if(isset($flight_passenger) && !empty($flight_passenger))
                    @foreach($flight_passenger as $paxKey => $paxVal)
                        ...
                        <tr>
                            ...
                            @if($baggageDisplay)<td>{{$dispBaggage}}</td>@endif
                ...
                @endforeach
                    @if($aFares['ssr_fare'] > 0)
                        <tr>
                            ...
                            <td>
                                {{Common::getRoundedFare($aFares['ssr_fare'] * $exchangeRate)}}
                            </td>
                        </tr>
                    @endif
                    @if($aFares['payment_charge'] > 0)
                    ...
                    <tr>
                        ...
                        <td>
                            {{Common::getRoundedFare(($aFares['total_fare'] + $aFares['payment_charge'] + $aFares['ssr_fare'] + $calcFare + $hst) * $exchangeRate) }}
                        </td>
                    </tr>
            </tbody>
        @endif
