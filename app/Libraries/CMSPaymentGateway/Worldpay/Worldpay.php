...
class Worldpay
{
    ...
    public function createOrder($order = array())
    {
        ...
        if (isset($response["orderCode"])) {
            ...
            return ErrorWp::throwError("apierror");
        }
    }
    ...
    public function authorize3DSOrder($orderCode, $responseCode)
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            return ErrorWp::throwError('ip', ErrorWp::$errors['3ds']['ordercode']);
        }
    ...
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            ErrorWp::throwError('ip', ErrorWp::$errors['capture']['ordercode']);
        }
    ...
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            ErrorWp::throwError('ip', ErrorWp::$errors['refund']['ordercode']);
        }
    ...
    {
        if (empty($orderCode) || !is_string($orderCode)) {
            ErrorWp::throwError('ip', ErrorWp::$errors['orderInput']['orderCode']);
        }
    ...
    public function getStoredCardDetails($token = false)
    {
        if (empty($token) || !is_string($token)) {
            ErrorWp::throwError('ip', ErrorWp::$errors['orderInput']['token']);
        }
