...
class PGWorldpay
{
	public static function authorize($paymentInput)
	{
		...
		if (isset($response['reply']['orderStatus']['reference']['content']) && !empty($response['reply']['orderStatus']['reference']['content'])) {
			...
			$encodedReturnUrl    = urlencode($paymentInput['pgReturnUrl']);
			
            $worlpayPaymentUrl    = $response['reply']['orderStatus']['reference']['content'].'&successURL='.$encodedReturnUrl.'&pendingURL='.$encodedReturnUrl.'&failureURL='.$encodedReturnUrl.'&cancelURL='.$encodedReturnUrl;
