...
class Connection {
    ...
    public function sendRequest($action, $json = false, $expectResponse = false, $method = 'POST')
    {
        ...
        if ($result === false) {
            if ($errno === 60) {
                ErrorWp::throwError('sslerror', false, $errno, null, $err);
            } elseif ($errno === 28) {
                ErrorWp::throwError('timeouterror', false, $errno, null, $err);
            } else {
                ErrorWp::throwError('uanv', false, $errno, null, $err);
            }
        }
        ...
        if ($expectResponse && ($response === null || $response === false )) {
            ErrorWp::throwError('uanv', ErrorWp::$errors['json'], 503);
        }
        ...
        if (isset($response["httpStatusCode"])) {
            ...
            if ($info['http_code'] != 200) {
                ErrorWp::throwError('apierror', $result, $info['http_code']);
            } else {
                $response = true;
            }
        }
